...
namespace CMS.UI.Services
{
    ...
    public class UserAuthApiService
    {
        ...
        {
            // Usar ASPNETCORE_ENVIRONMENT en lugar de "Environment" que puede estar mal configurado
            var environment = Environment.GetEnvironmentVariable("ASPNETCORE_ENVIRONMENT") ?? "Development";
            var baseUrl = _configuration[$"ApiSettings:{environment}:BaseUrl"];

            // Fallback explícito para desarrollo
            ...
            _logger.LogDebug("🌐 API Base URL: {BaseUrl} (Env: {Env})", baseUrl, environment);
            return baseUrl;
        }
        ...
        public async Task<UserCompanyAuthSummary?> GetAuthSummaryAsync(int userId, int companyId)
        {
            ...
            {
                ConfigureAuthHeader();
                // ⚠️ Ruta correcta: /api/users (plural)
                var url = $"{GetApiBaseUrl()}/api/users/{userId}/companies/{companyId}/auth";

                _logger.LogDebug("🔗 Llamando a: {Url}", url);
