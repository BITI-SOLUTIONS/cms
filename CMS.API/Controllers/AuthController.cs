...
namespace CMS.API.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        public async Task<ActionResult<CmsUserDto>> SyncUser([FromBody] AzureUserInfo model)
        {
            ...
            // Traer roles del usuario (de la primera compañía activa)
            var firstCompanyId = await _db.UserCompanies
                .Where(uc => uc.ID_USER == user.ID_USER && uc.IS_ACTIVE)
            ...
            var roles = await (from ucr in _db.UserCompanyRoles
                               ...
                               where ucr.ID_USER == user.ID_USER && ucr.ID_COMPANY == firstCompanyId
        ...
        public async Task<ActionResult<TokenResponseDto>> GenerateToken([FromBody] TokenRequestDto request)
        {
            try
            {
                ...
                // Verificar que se proporcione CompanyId
                if (request.CompanyId <= 0)
                {
                    _logger.LogWarning("❌ CompanyId no proporcionado para usuario {UserId}", user.ID_USER);
                    return BadRequest(new TokenResponseDto
                    {
                        Success = false,
                ...
                if (!hasAccess)
                {
                    _logger.LogWarning("❌ Usuario {UserId} sin acceso a compañía {CompanyId}", 
                        user.ID_USER, request.CompanyId);
                    return Unauthorized(new TokenResponseDto
                    {
                        Success = false,
                ...
                // ⭐ Calcular permisos del usuario EN ESTA COMPAÑÍA (nueva arquitectura)
                var permissions = await _permissionService.GetUserPermissionsForCompanyAsync(
                    user.ID_USER, request.CompanyId);

                // ⭐ Obtener roles del usuario EN ESTA COMPAÑÍA (nueva arquitectura)
                var roles = await _permissionService.GetUserRolesForCompanyAsync(
                    user.ID_USER, request.CompanyId);

                _logger.LogInformation("🔑 Token Azure AD para {UserId} en compañía {CompanyId}: {RoleCount} roles, {PermCount} permisos",
                    user.ID_USER, request.CompanyId, roles.Count, permissions.Count);

                // Generar token con roles y compañía
        ...
        public async Task<ActionResult<TokenResponseDto>> GenerateLocalToken([FromBody] LocalTokenRequestDto request)
        {
            try
            {
                ...
                _logger.LogInformation("🔑 Token local para {UserId} en compañía {CompanyId}: {RoleCount} roles, {PermCount} permisos",
                    user.ID_USER, request.CompanyId, roles.Count, permissions.Count);

                // Generar token incluyendo el CompanyId y roles
                var token = _jwtService.GenerateToken(
                    user.ID_USER,
                    user.USER_NAME,
                    user.EMAIL,
